...
```{r}
brms_model <- brm(formula = price_norm ~ (1|region_id/banner_id) - 1 + store_id + UPC,
              data = model_data, 
              family = gaussian(),
              prior = prior1,
              cores = 4,
...
```{r}
brms_model2 <- brm(formula = price_norm ~ region_id + banner_id + store_id + UPC - 1,
              data = model_data, 
              family = gaussian(),
              prior = prior1,
              cores = 4,
...
```{r}
loo(brms_model, brms_model2)
```

MLM fits better by an SE of 11.9. Means what?
